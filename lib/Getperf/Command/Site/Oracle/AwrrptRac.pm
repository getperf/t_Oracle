...
sub parse {
	...
	while (my $line = <IN>) {
        ...
        } elsif ($line=~/^Top Timed Background Events   /) {
    ...
    # Report "Top Timed Events"
    {
        ...
        my ($events, $header) = $self->parse_timed_event($lines, $sec);
        # print Dumper $events;
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
    ...
    # Report "Top Timed Background Events"
    {
        ...
        my ($events, $header) = $self->parse_background_event($lines, $sec);
        # print Dumper $events;
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/ora_bgevent_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'ora_bgevent_rac', $header);
            $data_info->pivot_report($output, \%data, $header);
        }
    }
