...
# Y3R Service check
#
# Check y3im08db01b server abailable.
# inet 10.152.16.37 netmask fffff800 broadcast 10.152.23.255
# inet 10.152.16.40 netmask fffff800 broadcast 10.152.23.255

	# CHECK_NET_IF=`/sbin/ifconfig -a | grep 10.152.16.37`
...
if [ ${SNAPSHOT_LEVEL} -ge 5 ]; then
	...
	ORACMD="${CWD}/chcsv $USER -i ${CWD}/sqlrank.sql -o ${DIR}/sqltop100_${SID}.txt ${OLD_ID} ${NEW_ID}"
#	echo ${ORACMD} >> ${ERR}
#	${ORACMD} >> ${ERR} 2>&1
...
if [ ${SNAPSHOT_LEVEL} -ge 7 ]; then
        ...
        ORACMD="${CWD}/chcsv $USER -i ${CWD}/objtop100_11g.sql -o ${DIR}/objrank_${SID}.txt ${OLD_ID} ${OLD_ID}"
	    ${ORACMD} >> ${ERR}
#        ${ORACMD} >> ${ERR} 2>&1
fi
