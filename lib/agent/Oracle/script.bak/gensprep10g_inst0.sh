...
if [ ${SNAPSHOT_LEVEL} -ge 5 ]; then
	...
	ORACMD="${CWD}/chcsv $USER -i ${CWD}/sqltop100_10g.sql -o ${DIR}/sqltop100_${SID}.txt ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} >> ${ERR} 2>&1"
	${ORACMD} >> ${ERR}
fi
...
if [ ${SNAPSHOT_LEVEL} -ge 7 ]; then
        ...
        ORACMD="${CWD}/chcsv $USER -i ${CWD}/objrank.sql -o ${DIR}/objrank_${SID}.txt ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} >> ${ERR} 2>&1"
        ${ORACMD} >> ${ERR}
fi
