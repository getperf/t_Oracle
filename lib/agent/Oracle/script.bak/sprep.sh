...
if [ ${SNAPSHOT_LEVEL} -ge 5 ]; then
        ...
        ORASQL="${CWD}/${SCRIPT}/sqltop100.sql"
        ORACMD="${CHCSV} $USER -i ${ORASQL} -o ${DIR}/sqltop100_${SID}.txt ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID}"
        ${ORACMD} > ${ERR} 2>&1
  if [ 0 != $? ]; then
...
if [ ${SNAPSHOT_LEVEL} -ge 7 ]; then
        ...
        ORASQL="${CWD}/${SCRIPT}/objrank.sql"
        ORACMD="${CHCSV} $USER -i ${ORASQL} -o ${DIR}/objrank_${SID}.txt ${OLD_ID} ${NEW_ID} ${OLD_ID} ${NEW_ID}"
        ${ORACMD} > ${ERR} 2>&1
  if [ 0 != $? ]; then
    echo "ERROR[chcsv] : ${ORACMD}"
    cat $ERR
